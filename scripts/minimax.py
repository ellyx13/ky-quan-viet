...
def evaluate(state, score):
    """
    Tính điểm dựa trên trạng thái bàn cờ và điểm số hiện tại.
    """
    player1_score = len(score["player1"]) - 1  # Số quân player1 đã ăn
    player2_score = len(score["player2"]) - 1  # Số quân player2 đã ăn
    return player2_score - player1_score  # Điểm cao hơn có lợi cho AI
    ...
    if maximizing_player:
        ...
        for move in range(1, 6):  # Player 2 (AI) chơi từ ô 1 -> 5
            if state[move] != "":  # Bỏ qua các ô trống
                for direction in ["left", "right"]:  # Thử cả hai hướng
                    new_state = apply_move(state, move, direction, score)
                    eval, _ = minimax(new_state, depth - 1, False, alpha, beta, score)
                    if eval > max_eval:
                        max_eval = eval
        ...
        for move in range(7, 12):  # Player 1 chơi từ ô 7 -> 11
            if state[move] != "":  # Bỏ qua các ô trống
                for direction in ["left", "right"]:  # Thử cả hai hướng
                    new_state = apply_move(state, move, direction, score)
                    eval, _ = minimax(new_state, depth - 1, True, alpha, beta, score)
                    if eval < min_eval:
                        min_eval = eval
                        if direction == "right":
                            best_move = (move, "ccw")
...
while True:
    ...
    if is_empty(state, "player2"):
        ...
        _, best_move = minimax(state, depth, True, float("-inf"), float("inf"), temp_score)
